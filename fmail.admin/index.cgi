...
if(&loginCheck){
	...
	if($flag{'ajax'}){
		...
		$include_path_date_param = "";
		if($form{'m'} eq 'users' && $form{'a'} eq 'form') {
			$include_path_date_param .= "?d=20230309"; # 2023.3.9 ver5.043
		}

		if(-f $include_path){
			$include_tag .= "<script type=\"text/javascript\" src=\"modules/" . $form{'m'} . "/include.js${include_path_date_param}\"></script>";
		}
		$include_path = "./modules/" . $form{'m'} . '/include.css';
		if(-f $include_path){
			$include_tag .= "<link rel=\"stylesheet\" href=\"modules/" . $form{'m'} . "/include.css${include_path_date_param}\" type=\"text/css\" />";
		}
		&display();
